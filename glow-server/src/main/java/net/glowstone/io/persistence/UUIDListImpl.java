...
public class UUIDListImpl implements UUIDList {
    ...
    @Inject
    public UUIDListImpl(PersistenceManager persistenceManager) {
    ...
    public List<PlayerProfile> getProfiles() {
        ...
        DatabaseResult<ResultSet> databaseResult = persistenceManager.executeQuery(query, Lists.newArrayList(tablename));
    ...
    public boolean containsUuid(UUID uuid) {
        ...
        String query = "SELECT uuid FROM ? WHERE uuid = \'?\';";
        DatabaseResult<ResultSet> databaseResult = persistenceManager.executeQuery(query, Lists.newArrayList(tablename, uuid.toString()));
        ResultSet result = databaseResult.getResults();
        boolean flag = false;
    ...
    public void add(OfflinePlayer player) {
        String query = "INSERT INTO ? (name, uuid) VALUES (\'?\', \'?\');";
        DatabaseResult<Integer> databaseResult = persistenceManager.executeUpdate(query, Lists.newArrayList(tablename, player.getName(), player.getUniqueId().toString()));
    ...
    public void remove(PlayerProfile profile) {
        String query = "DELETE FROM ? WHERE uuid = \'?\';";
        DatabaseResult<Integer> databaseResult = persistenceManager.executeUpdate(query, Lists.newArrayList(tablename, profile.getId().toString()));
        if(databaseResult.getResults() != 1) {
